...
from automonai.core.train import get_metrics, compute_metrics, get_metric_values, train_one_epoch
...
class TestGetMetricsAllTypes:
    """Test that all supported metrics can be created and return scalars."""

    # Metrics that work with any num_classes
    STANDARD_METRICS = [
        "dice",
        "iou",
        "hausdorff_distance",
    ...
    # panoptic_quality only supports binary (num_classes=2)
    BINARY_ONLY_METRICS = ["panoptic_quality"]
    ...
    def test_metric_creation(self, metric_name):
        """Test that each metric can be created without error."""
        metrics = get_metrics([metric_name], num_classes=3)
        assert metric_name in metrics
        assert metrics[metric_name] is not None

    @pytest.mark.parametrize("metric_name", STANDARD_METRICS)
    def test_metric_returns_scalar(self, metric_name):
        """Test that each metric computes and returns a float scalar."""
        num_classes = 3
        metrics = get_metrics([metric_name], num_classes=num_classes)
        ...
        ), f"{metric_name} returned {type(results[metric_name])}, expected float"

    def test_panoptic_quality_binary(self):
        """Test panoptic_quality with binary (2-class) input."""
        num_classes = 2
        metrics = get_metrics(["panoptic_quality"], num_classes=num_classes)
        ...
        assert isinstance(results["panoptic_quality"], float)

    def test_panoptic_quality_skipped_multiclass(self):
        """Test panoptic_quality is skipped for multi-class (>2) input."""
        metrics = get_metrics(["panoptic_quality"], num_classes=3)
        assert "panoptic_quality" not in metrics

    def test_all_standard_metrics_together(self):
        """Test creating and computing all standard metrics in a single call."""
        num_classes = 3
        metrics = get_metrics(self.STANDARD_METRICS, num_classes=num_classes)
        ...
        for name in self.STANDARD_METRICS:
            assert name in results, f"Missing metric: {name}"
            assert isinstance(
                results[name], float
            ), f"{name} returned {type(results[name])}, expected float"
...
class TestDeepSupervisionLoss:
    """Test that DeepSupervisionLoss works with integer label tensors."""

    def test_deep_supervision_with_long_labels(self):
        """DeepSupervisionLoss must handle long/int labels without error.

        Regression test: F.interpolate doesn't support Long tensors, so
        labels must be cast to float before passing to DeepSupervisionLoss.
        """
        from monai.losses import DeepSupervisionLoss

        base_loss = DiceLoss(to_onehot_y=True, softmax=True)
        ds_loss = DeepSupervisionLoss(base_loss)

        num_classes = 2
        # Simulate multi-scale outputs from deep supervision
        out_full = torch.rand(2, num_classes, 16, 16)
        out_half = torch.rand(2, num_classes, 8, 8)
        outputs = [out_full, out_half]

        labels = torch.randint(0, num_classes, (2, 1, 16, 16))

        # This would raise NotImplementedError with labels.long()
        loss = ds_loss(outputs, labels.float())
...
class TestTrainOneEpoch:
    ...
    def test_returns_both_metrics(self, fabric, setup):
        ...
        model, loader, loss_fn, optimizer = setup
        result = train_one_epoch(fabric, model, loader, loss_fn, optimizer, metrics=["dice", "iou"])
    ...
    def test_model_weights_change(self, fabric, setup):
        ...
        changed = any(
            not torch.equal(init, final) for init, final in zip(initial_params, final_params)
        )
